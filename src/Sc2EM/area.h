...
public:
	...
	const Sc2Bindings::TilePosition &		TopLeft() const					{ return m_topLeft ; }
	const Sc2Bindings::TilePosition &		BottomRight() const				{ return m_bottomRight ; }
	Sc2Bindings::TilePosition				BoundingBoxSize() const;

	// Position of the MiniTile with the highest Altitude() value.
	const Sc2Bindings::WalkPosition &		Top() const						{ return m_top; }
	...
	// Note: only a call to Map::OnMineralDestroyed(sc2::Unit u) may change the result (by removing eventually one element).
...
//	Details: The functions below are used by the BWEM's internals
									...
									Area(detail::Graph * pGraph, id areaId, Sc2Bindings::WalkPosition top, int miniTiles);
	...
	void							AddTileInformation(const Sc2Bindings::TilePosition t, const Tile & tile);
...
private:
	...
	int								ComputeBaseLocationScore(Sc2Bindings::TilePosition location) const;
	bool							ValidateBaseLocation(Sc2Bindings::TilePosition location, std::vector<Mineral *> & BlockingMinerals) const;
	std::vector<int>				ComputeDistances(Sc2Bindings::TilePosition start, const std::vector<Sc2Bindings::TilePosition> & Targets) const;
