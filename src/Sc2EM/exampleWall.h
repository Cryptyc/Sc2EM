...
public:
	enum class tight_t {zealot, zergling};
								...
								ExampleWall(const Map & theMap, const ChokePoint * cp, DebugInterface* inDebug, tight_t tight = tight_t::zealot);
	...
	const std::vector<sc2::UnitTypeID> &		BuildingTypes() const	{ return m_BuildingTypes; }

protected:
	void						Compute(int wallSize, const std::vector<Sc2Bindings::TilePosition> & BuildableBorderTiles1,
													  const std::vector<Sc2Bindings::TilePosition> & BuildableBorderTiles2);
	...
	tight_t								m_tight;
	Sc2Bindings::Position						m_center = Sc2Bindings::Positions::None;
	std::vector<Sc2Bindings::TilePosition>	m_Locations;
	std::vector<sc2::UnitTypeID>		m_BuildingTypes;
	DebugInterface* Debug;
};


std::vector<ExampleWall> findWalls(const Map & theMap, DebugInterface* Debug);
