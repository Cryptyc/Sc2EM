...
void DebugDrawBox(sc2::DebugInterface* Debug, Position Start, Position Size, Sc2Bindings::Color Color, bool IsSolid = true)
{
	Debug->DebugBoxOut(Point3D(Start.x, Start.y, DRAW_HIGHT), Point3D(Start.x + Size.x, Start.y + Size.y, DRAW_HIGHT), BWToSc2Color(Color));
}

void DebugDrawTriangle(sc2::DebugInterface* Debug, Position Corner1, Position Corner2, Position Corner3, Sc2Bindings::Color Color)
{
	Debug->DebugLineOut(Point3D(Corner1.x, Corner1.y, DRAW_HIGHT), Point3D(Corner2.x, Corner2.y, DRAW_HIGHT), BWToSc2Color(Color));
	Debug->DebugLineOut(Point3D(Corner2.x, Corner2.y, DRAW_HIGHT), Point3D(Corner3.x, Corner3.y, DRAW_HIGHT), BWToSc2Color(Color));
	Debug->DebugLineOut(Point3D(Corner3.x, Corner3.y, DRAW_HIGHT), Point3D(Corner1.x, Corner1.y, DRAW_HIGHT), BWToSc2Color(Color));
}

void DebugDrawLine(sc2::DebugInterface* Debug, Position Corner1, Position Corner2, Sc2Bindings::Color Color)
{
	Debug->DebugLineOut(Point3D(Corner1.x, Corner1.y, DRAW_HIGHT), Point3D(Corner2.x, Corner2.y, DRAW_HIGHT), BWToSc2Color(Color));
}
...
void drawMap(const Map & theMap, sc2::DebugInterface* Debug)
{
	...
	for (int x = 0 ; x < theMap.Size().x ; ++x)
	{
		...
		if (MapDrawer::showUnbuildable && !tile.Buildable())
			drawDiagonalCrossMap(Position(t), Position(t + 1), MapDrawer::Color::unbuildable, Debug);
		...
		if (MapDrawer::showGroundHeight && (tile.GroundHeight() > 0))
		{
			...
			DebugDrawBox(Debug, Position(t), Position(t)+6, col, bool("isSolid"));
			if (tile.Doodad())  DebugDrawTriangle(Debug, center(t) + Position(0, 5), center(t) + Position(-3, 2), center(t) + Position(+3, 2), Sc2Bindings::Colors::White);
		}
	}
	...
	for (int x = 0 ; x < theMap.WalkSize().x ; ++x)
	{
		...
		if (MapDrawer::showSeas && miniTile.Sea())
			drawDiagonalCrossMap(Position(w), Position(w + 1), MapDrawer::Color::sea, Debug);
		
		if (MapDrawer::showLakes && miniTile.Lake())
			drawDiagonalCrossMap(Position(w), Position(w + 1), MapDrawer::Color::lakes, Debug);
	}
		...
		for (const Area & area : theMap.Areas())
			for (const ChokePoint * cp : area.ChokePoints())
				for (ChokePoint::node end : {ChokePoint::end1, ChokePoint::end2})
					DebugDrawLine(Debug, Position(cp->Pos(ChokePoint::middle)), Position(cp->Pos(end)), MapDrawer::Color::cp);

	if (MapDrawer::showMinerals)
		for (auto & m : theMap.Minerals())
		{
			DebugDrawBox(Debug, Position(m->TopLeft()), Position(m->TopLeft() + m->Size()) , MapDrawer::Color::minerals);
			if (m->Blocking())
				drawDiagonalCrossMap(Position(m->TopLeft()), Position(m->TopLeft() + m->Size()), MapDrawer::Color::minerals, Debug);
		}

	if (MapDrawer::showGeysers)
		for (auto & g : theMap.Geysers())
			DebugDrawBox(Debug, Position(g->TopLeft()), Position(g->TopLeft() + g->Size()) , MapDrawer::Color::geysers);

	if (MapDrawer::showStaticBuildings)
		for (auto & s : theMap.StaticBuildings())
		{
			DebugDrawBox(Debug, Position(s->TopLeft()), Position(s->TopLeft() + s->Size()) , MapDrawer::Color::staticBuildings);
			if (s->Blocking())
				drawDiagonalCrossMap(Position(s->TopLeft()), Position(s->TopLeft() + s->Size()), MapDrawer::Color::staticBuildings, Debug);
		}
	...
	for (const Area & area : theMap.Areas())
		for (const Base & base : area.Bases())
		{
			if (MapDrawer::showBases)
				DebugDrawBox(Debug, Position(base.Location()), Position(base.Location() + Sc2UnitTypes::getInstance().GetTileSize(UNIT_TYPEID::TERRAN_COMMANDCENTER) ), MapDrawer::Color::bases);
			...
			if (MapDrawer::showAssignedRessources)
			{
				...
				for (const Ressource * r : AssignedRessources)
					DebugDrawLine(Debug, base.Center(), r->Pos(), MapDrawer::Color::assignedRessources);
			}
		}
}
...
//				MapPrinter::Get().Circle(WalkPosition(TilePosition(x, y)) + 2, 4, MapPrinter::Color(255, 255, 255));
	...
	if (MapPrinter::showStartingLocations)
		for (TilePosition t : theMap.StartingLocations())
		{
			...
			WalkPosition size(Sc2UnitTypes::getInstance().GetTileSize(UNIT_TYPEID::TERRAN_COMMANDCENTER));	// same size for other races
...
public:
	...
	vector<const Unit*>		GetUnits(TilePosition topLeft, TilePosition bottomRight, sc2::Unit::Alliance player) const;
};


void SimpleGridMap::Add(const Unit *unit)
{
	auto & List = GetCell(TilePosition(Sc2Bindings::TilePositionFromPoint3D(unit->pos))).Units;

	if (!contains(List, unit)) List.push_back(unit);
}


void SimpleGridMap::Remove(const Unit *unit)
{
	auto & List = GetCell(TilePosition(Sc2Bindings::TilePositionFromPoint3D(unit->pos))).Units;
...
vector<const Unit*> SimpleGridMap::GetUnits(TilePosition topLeft, TilePosition bottomRight, sc2::Unit::Alliance player) const
{
	...
	for (int j = j1; j <= j2; ++j)
	{
		for (int i = i1; i <= i2; ++i)
		{
			for (const Unit* unit : GetCell(i, j).Units)
			{
				...
				{
					if (sc2_ext::inBoundingBox(TilePosition(Sc2Bindings::TilePositionFromPoint3D(unit->pos)), topLeft, bottomRight))
					{
						Res.push_back(unit);
					}
				}
...
void gridMapExample(const Map & theMap, const sc2::ObservationInterface *obs)
{
	...
	for (const sc2::Unit* unit : obs->GetUnits())
	{
		Grid.Add(unit);
	}
	...
	for (const sc2::Unit* unit : Grid.GetUnits(centerTile - 10, centerTile + 10, sc2::Unit::Alliance::Self))
	{
		std::cout << "My " << unit->unit_type <<  " is near the center of the map." << endl;
	}
}
